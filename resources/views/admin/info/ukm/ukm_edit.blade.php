...
@section('content')
<form action="/ukm/{{ $detail->token_ukm }}/edit" method="POST" enctype="multipart/form-data" novalidate class="card">
    @csrf
    ...
    <div class="card-body row">
      <div class="row col-lg-6 g-3 m-0">
        <div class="col-lg-12 mt-0">
          ...
          <input type="text" name="nama_ukm" id="nama_ukm" value="{{ $detail->nama_ukm }}" class="form-control">
        </div>
          ...
          <div class="col-sm-12 ">
            <label for="input-file" id="drop-area">
              ...
              <div id="image-view" style="height: 200px; width: 200px">
                <img src="{{ asset('image/ukm/' . $detail->logo_ukm) }}" width="100%" height="100%">
              </div>
            </label>
          </div>
        </div>
          ...
          @foreach ($kategoriUkms as $item)
            <label class="form-check-label p-2 pt-0 pb-0 mt-0" for="{{ $item->nama_kategori }}">
              <input name="kategori[]" class="form-check-input" type="checkbox" value="{{ $item->id }}"
                ...
                {{ $detail->kategoriukm->contains($item->id) ? 'checked' : '' }}>
              {{ $item->nama_kategori }}
            </label>
          @endforeach
        </div>
      ...
      <div class="row col-lg-12 mt-4">
        <div class="col-lg-6">
          ...
          <input type="text" name="link_pendaftaran" id="link_pendaftaran" value="{{ $detail->link_pendaftaran }}"
            class="form-control">
        </div>
        <div class="col-lg-6">
          ...
          <select name="status_ukm" id="status_ukm" class="form-control">
            ...
            <option value="1" {{ $detail->status_ukm == 1 ? 'selected' : '' }}>Aktif</option>
            <option value="0" {{ $detail->status_ukm == 0 ? 'selected' : '' }}>Non Aktif</option>
          </select>
        </div>
      </div>
      <div class="col-lg-12 mt-3">
        <Label for="deskripsi_ukm" class="form-label">Deskripsi UKM</Label>
        <textarea name="deskripsi_ukm" id="editor" required>{{ $detail->deskripsi_ukm }}</textarea>
      </div>
    </div>
